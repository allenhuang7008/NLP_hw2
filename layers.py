...
def attention(query, key, value, mask=None, dropout=None):
    ...
    if mask is not None:
        score = score.masked_fill(mask.unsqueeze(1) == 0, -1e9) # mask unwanted tokens to extremely small value, so its softmax weight is 0
    
    # compute softmax weight
    attn_weights = torch.softmax(score, dim=-1)

    # dropout weights for regularization
    if dropout is not None:
        attn_weights = dropout(attn_weights)

    # compute attention output
    attn_out = torch.matmul(attn_weights, value) #output tensor (N, Lq, d_v)
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        attn_out, self.attn = attention(query, key, value, mask=mask, dropout=self.dropout) 
        #print(attn_out.size())

        #concate the heads then apply the final linear layer
