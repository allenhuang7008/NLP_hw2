...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout, max_len=5000):
        ...
        dim_input = torch.arange(0, d_model, 2, dtype=torch.float)
        pe[:, 0::2] = torch.sin(position / (10000**(dim_input / d_model)))
        pe[:, 1::2] = torch.cos(position / (10000**(dim_input / d_model)))
